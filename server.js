  ...
  try
  {
    ...
    else if (req.query.guess == req.session.answer)
    {
      ...
      con.connect(function(err) 
      {
        ...
        {
          con.query('INSERT INTO GAMEDATA (GAME_SCORE) VALUES (?)', [req.session.guesses], function (err, result, fields) 
          {
            if (err) 
              writeResult(req, res, {'error' : err});
            else
...
function stats(req, res)
{
  ...
  con.connect(function(err) 
  {
    ...
    {
      con.query("SELECT MAX(GAME_SCORE) AS 'best', MIN(GAME_SCORE) AS 'worst', COUNT(GAME_SCORE) AS 'gamesPlayed' FROM GAMEDATA", function (err, result, fields) 
      {
        if (err) 
          writeResult(req, res, {'error' : err});
        else
